...
def test_map_timeID():
    ...
    ids = preprocessing.RollingWindow.map_timeId(datetimes)
...
def test_get_rolled():
    ...
    feature_window = 5
    forecast_horizon = 3
    roll = preprocessing.RollingWindow(df, feature_window, forecast_horizon)
    assert len(df_rolled) = df['id'].nunique() * feature_window * (len(df[df.id =='AXP']) - feature_window - forecast_horizon + 1)
    assert df_rolled.loc[df_rolled['id'] =='AXP', 'ref_date'].min() == df.loc[df['id'] == 'AXP', 'datetime'].sort_values().unique()[feature_window-1]
